...
class Algorithm:
    ...
    def run_pipeline_step(self, script_name, step_name, input_paths=None, source_name=None):
        ...
        try:
            ...
            # Set environment variables for input paths and source name if provided
            env = {}
            if input_paths:
    ...
    def ensure_working_directories(self):
        ...
        directories = [
            ...
            Path(os.environ.get("OCR_CACHE_DIR", "/shared_volumes/ocr_cache"))
        ]
        ...
        for input_file in input_files:
            ...
            try:
                ...
                with zipfile.ZipFile(file_path, 'r') as zip_ref:
                    ...
                    print(f"� Found {total_files} PDF files in ZIP")
                    ...
                    for pdf_entry in pdf_entries:
                        try:
                            ...
                            print(f"Progress: ({total_processed+total_failed}/{total_files}) — { total_files - total_processed - total_failed} left")
    ...
    def process_single_pdf(self, pdf_path, source_name=None):
        ...
        try:
            ...
            try:
                ocr_success = self.run_pipeline_step("ocr/ocr_processor.py", f"[OCR] || for {display_name}", [pdf_path], source_name)
                if not ocr_success:
